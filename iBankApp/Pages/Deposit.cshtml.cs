...
namespace IBankApp.Pages
{
    public class DepositModel : PageModel
    {
        ...
        public DepositModel(HttpClient httpClient)
        {
        ...
        public async Task<IActionResult> OnPost()
        {
            ...
            try
            {
                ...
                var requestBody = new DepositRequest(Amount, Message, Account);
                var json = JsonConvert.SerializeObject(requestBody);
                var content = new StringContent(json, Encoding.UTF8, "application/json");

                var response = await _httpClient.PostAsync("https://starfish-app-g96va.ondigitalocean.app/v1/transactions/deposit", content);
                ...
                {
                    var responseString = await response.Content.ReadAsStringAsync();
                    var depositResponse = JsonConvert.DeserializeObject<DepositResponse>(responseString);
                    ErrorMessage = depositResponse.error;
                }
            }
            catch (Exception ex)
            {
                ErrorMessage = $"Error on Deposit transaction!";
            }
            return Page();
        }
